...
/**
 * Scans an array of candidate venues and returns the best deduplication
 * decision for the incoming venue.
 *
 * Priority: merge > review > keep_separate
 * - Returns the first merge decision found.
 * - Returns the first review decision if no merge found.
 * - Returns keep_separate if no candidates pass any signal.
 */
export function findBestMatch(
  incoming: VenueForDedup,
  candidates: VenueForDedup[]
): DedupeDecision {
  let firstReview: DedupeDecision | null = null;

  for (const candidate of candidates) {
    const decision = scoreVenueCandidate(incoming, candidate);

    if (decision.action === 'merge') {
      return decision; // Merge is highest priority — return immediately
    }

    if (decision.action === 'review' && firstReview === null) {
