...
export default function ZoomControls({ activeProvince, boxZoomActive, onToggleBoxZoom }: ZoomControlsProps) {
  const map = useMap();
  ...
  return (
    <div className="absolute bottom-6 right-3 z-[1000] flex flex-col gap-1.5">
      ...
      <div className="bg-white/95 backdrop-blur-sm rounded-lg shadow-md border border-gray-200 flex flex-col divide-y divide-gray-100">
        <button onClick={zoomIn} title="Zoom in" aria-label="Zoom in" className={`${btnClass()} rounded-t-lg`}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
            <line x1="12" y1="5" x2="12" y2="19" />
            <line x1="5" y1="12" x2="19" y2="12" />
          </svg>
        </button>
        <button onClick={zoomOut} title="Zoom out" aria-label="Zoom out" className={btnClass()}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
            <line x1="5" y1="12" x2="19" y2="12" />
          </svg>
        </button>
        ...
        <button onClick={resetView} title="Reset to all provinces" aria-label="Reset view" className={btnClass()}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {typeof navigator !== 'undefined' && navigator.geolocation && (
          <button onClick={handleGeolocation} title="My location" aria-label="Center on my location" className={`${btnClass()} rounded-b-lg`}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="3" />
