/**
 * Characterization tests for src/app/api/cron/discover-places-nl/route.ts
 *
 * GET /api/cron/discover-places-nl:
 * - 401 without valid CRON_SECRET bearer token
 * - 200 with valid token, calls runPlacesDiscovery(PLACES_CITIES.NL)
 * - 500 when runPlacesDiscovery throws
 */

const CRON_SECRET = 'test-cron-secret-nl';
...
function makeRequest(authHeader?: string) {
  return new Request('http://localhost/api/cron/discover-places-nl', {
    method: 'GET',
    headers: authHeader ? { authorization: authHeader } : {},
  }) as unknown as Parameters<typeof GET>[0];
}

describe('GET /api/cron/discover-places-nl', () => {
  ...
  describe('with valid auth', () => {
    ...
    it('calls runPlacesDiscovery with NL cities', async () => {
      await GET(makeRequest(auth()));
      expect(mockRunPlacesDiscovery).toHaveBeenCalledWith(
        expect.arrayContaining(["St. John's"])
      );
    });
    ...
    it('logs the discovery run with province NL', async () => {
      await GET(makeRequest(auth()));
  ...
  describe('error handling', () => {
    it('returns 500 when runPlacesDiscovery throws', async () => {
      mockRunPlacesDiscovery.mockRejectedValue(new Error('NL API error'));

      const res = await GET(makeRequest(`Bearer ${CRON_SECRET}`));
      expect(res.status).toBe(500);
      const body = await res.json();
      expect(body.success).toBe(false);
      expect(body.error).toContain('NL API error');
    });
  });
});
