/**
 * Characterization tests for src/app/api/cron/discover-places-nb/route.ts
 *
 * GET /api/cron/discover-places-nb:
 * - 401 without valid CRON_SECRET bearer token
 * - 200 with valid token, calls runPlacesDiscovery(PLACES_CITIES.NB) and logs run
 * - 500 when runPlacesDiscovery throws
 */

const CRON_SECRET = 'test-cron-secret-nb';
...
function makeRequest(authHeader?: string) {
  return new Request('http://localhost/api/cron/discover-places-nb', {
    method: 'GET',
    headers: authHeader ? { authorization: authHeader } : {},
  }) as unknown as Parameters<typeof GET>[0];
}

describe('GET /api/cron/discover-places-nb', () => {
  ...
  describe('with valid auth', () => {
    ...
    it('calls runPlacesDiscovery with NB cities', async () => {
      await GET(makeRequest(auth()));
      expect(mockRunPlacesDiscovery).toHaveBeenCalledWith(
        expect.arrayContaining(['Fredericton', 'Moncton', 'Saint John'])
      );
    });

    it('returns discovery result fields in response', async () => {
      const res = await GET(makeRequest(auth()));
      const body = await res.json();
    ...
    it('logs the discovery run to db.insert', async () => {
      await GET(makeRequest(auth()));
    ...
    it('does not call runPlacesDiscovery when rate limited by auth', async () => {
      // Without auth, orchestrator should not run
  ...
  describe('error handling', () => {
    it('returns 500 when runPlacesDiscovery throws', async () => {
      mockRunPlacesDiscovery.mockRejectedValue(new Error('Places API down'));

      const res = await GET(makeRequest(`Bearer ${CRON_SECRET}`));
      expect(res.status).toBe(500);
      const body = await res.json();
      expect(body.success).toBe(false);
      expect(body.error).toContain('Places API down');
    });
  });
});
